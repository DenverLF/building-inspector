...
export default async function DashboardPage() {
  ...
  try {
    ...
    const [tasksRes, noticesRes, activityRes] = await Promise.all([
      ...
      supabase.from('notices').select('status, remedy_deadline, created_at').neq('status', 'resolved'),
    ...
    if (noticesRes.data) {
      for (const n of noticesRes.data) {
        ...
        if (n.remedy_deadline && n.remedy_deadline < today) overdueNoticesCount++
        if (n.status === 'draft' && n.created_at < staleDate) notificationCount++
      }
    }

    notificationCount += overdueCount + todayCount + overdueNoticesCount
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#1a1745] px-5 pt-10 pb-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/dashboard/notifications" className="relative w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors">
